...
func TestDesiredNodeExporterService(t *testing.T) {
	t.Run("should return the default service", func(t *testing.T) {
		...
		expected := v1.Service{
			ObjectMeta: metav1.ObjectMeta{
				Name:        fmt.Sprintf("%s-service", ComponentNodeExporter),
	...
	t.Run("should return a service object with overridden values", func(t *testing.T) {
		params := manifests.Params{
			Config: config.Config{},
			Log:    logger,
			NodeExp: v1alpha1.NodeExporter{
