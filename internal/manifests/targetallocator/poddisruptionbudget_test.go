...
func TestPDBWithValidStrategy(t *testing.T) {
	for _, test := range tests {
		for _, strategy := range []v1alpha1.TargetAllocatorAllocationStrategy{v1alpha1.TargetAllocatorAllocationStrategyPerNode, v1alpha1.TargetAllocatorAllocationStrategyConsistentHashing} {
		...
		t.Run(test.name, func(t *testing.T) {
			targetAllocator := v1alpha1.TargetAllocator{
				...
				Spec: v1alpha1.TargetAllocatorSpec{
					...
					AllocationStrategy: v1alpha1.TargetAllocatorAllocationStrategyLeastWeighted,
				},
			}
			configuration := config.New()
			pdb, err := PodDisruptionBudget(Params{
