...
type NodeExporterSpec struct {
	// Resources to set on the Node Exporter pods.
	// +optional
	...
	// Image indicates the container image to use for the Node Exporter.
	// +optional
	...
	// ENV vars to set on the Node Exporter Pods. These can then in certain cases be
...
type NodeExporterStatus struct {
	// Scale is the NodeExporter's scale subresource status.
	// +optional
	Scale ScaleSubresourceStatus `json:"scale,omitempty"`

	// Version of the managed Node Exporter (operand)
	// +optional
	...
	// +optional
	// Deprecated: use "NodeExporter.Status.Scale.Replicas" instead.
