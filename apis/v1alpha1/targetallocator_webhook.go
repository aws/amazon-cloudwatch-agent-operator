...
// //+kubebuilder:webhook:path=/mutate-cloudwatch-aws-amazon-com-v1alpha1-targetallocator,mutating=true,failurePolicy=fail,groups=cloudwatch.aws.amazon.com,resources=targetallocators,verbs=create;update,versions=v1alpha1,name=mtargetallocatoralpha.kb.io,sideEffects=none,admissionReviewVersions=v1
// //+kubebuilder:webhook:verbs=create;update,path=/validate-cloudwatch-aws-amazon-com-v1alpha1-targetallocator,mutating=false,failurePolicy=fail,groups=cloudwatch.aws.amazon.com,resources=targetallocators,versions=v1alpha1,name=vtargetallocatorcreateupdatealpha.kb.io,sideEffects=none,admissionReviewVersions=v1
// //+kubebuilder:webhook:verbs=delete,path=/validate-cloudwatch-aws-amazon-com-v1alpha1-targetallocator,mutating=false,failurePolicy=ignore,groups=cloudwatch.aws.amazon.com,resources=targetallocators,versions=v1alpha1,name=vtargetallocatordeletealpha.kb.io,sideEffects=none,admissionReviewVersions=v1
...
func (w TargetAllocatorWebhook) defaulter(ta *TargetAllocator) error {
	...
	if ta.Spec.AllocationStrategy == TargetAllocatorAllocationStrategyConsistentHashing &&
		ta.Spec.PodDisruptionBudget == nil {
		ta.Spec.PodDisruptionBudget = &PodDisruptionBudgetSpec{
...
func (w TargetAllocatorWebhook) validate(ctx context.Context, ta *TargetAllocator) (admission.Warnings, error) {
	...
	if ta.Spec.PrometheusCR.Enabled {
		warnings, err := CheckTargetAllocatorPrometheusCRPolicyRules(ctx, w.reviewer, ta.Spec.ServiceAccount, ta.GetNamespace())
...
func SetupTargetAllocatorWebhook(mgr ctrl.Manager, cfg config.Config, reviewer *rbac.Reviewer) error {
	cvw := &TargetAllocatorWebhook{
		reviewer: reviewer,
		logger:   mgr.GetLogger().WithValues("handler", "TargetAllocatorWebhook", "version", "v1alpha1"),
