...
var _ conversion.Convertible = &AmazonCloudWatchAgent{}

func (src *AmazonCloudWatchAgent) ConvertTo(dstRaw conversion.Hub) error {
	switch t := dstRaw.(type) {
	case *v1beta1.AmazonCloudWatchAgent:
		dst := dstRaw.(*v1beta1.AmazonCloudWatchAgent)
...
func (dst *AmazonCloudWatchAgent) ConvertFrom(srcRaw conversion.Hub) error {
	switch t := srcRaw.(type) {
	case *v1beta1.AmazonCloudWatchAgent:
		src := srcRaw.(*v1beta1.AmazonCloudWatchAgent)
...
func tov1beta1(in AmazonCloudWatchAgent) (v1beta1.AmazonCloudWatchAgent, error) {
	copy := in.DeepCopy()
	cfg := ""
	if err := yaml.Unmarshal([]byte(copy.Spec.Config), cfg); err != nil {
		return v1beta1.AmazonCloudWatchAgent{}, errors.New("could not convert config json to v1beta1.Config")
	}

	return v1beta1.AmazonCloudWatchAgent{
		ObjectMeta: copy.ObjectMeta,
		Status: v1beta1.AmazonCloudWatchAgentStatus{
...
func tov1alpha1(in v1beta1.AmazonCloudWatchAgent) (*AmazonCloudWatchAgent, error) {
	copy := in.DeepCopy()
	configYaml := copy.Spec.Config

	return &AmazonCloudWatchAgent{
		ObjectMeta: copy.ObjectMeta,
		Status: AmazonCloudWatchAgentStatus{
