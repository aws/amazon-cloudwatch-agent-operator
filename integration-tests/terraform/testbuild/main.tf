...
resource "null_resource" "apply_yaml_files" {
  triggers = {
    # Add any changes to your YAML files that should trigger re-execution
    file_sha = sha1(file("../sample-instrumentation.yaml")) # Update with your file paths
  }
  ...
  provisioner "local-exec" {
    command = <<-EOT
      ...
      sleep 60
      # Add other kubectl commands here as needed
    EOT
  }
}

# Additional null_resource blocks for other actions

# Add additional null_resource blocks for other actions, each with their provisioners

# Example of checking the status of pods
resource "null_resource" "check_pod_status" {
  depends_on = [null_resource.apply_yaml_files]
