...
public class CWMetricValidator implements IValidator {
        ...
        () -> {
          List<Metric> metricList =
              this.listMetricFromCloudWatch(
                  cloudWatchService, expectedMetricList, context.getServiceName());

            if (context.getRemoteServiceName() != null && !context.getRemoteServiceName().isEmpty()) {
                metricList.addAll(
                    this.listMetricFromCloudWatch(
                        cloudWatchService, expectedMetricList, context.getRemoteServiceName()));
            }
  ...
  private List<Metric> listMetricFromCloudWatch(
      CloudWatchService cloudWatchService, List<Metric> expectedMetricList, String serviceName)
      throws IOException {
    ...
    for (String metricName : metricNameMap.keySet()) {
      result.addAll(
          cloudWatchService.listMetrics(metricNameMap.get(metricName), metricName, serviceName));
    }
    return result;
  }
